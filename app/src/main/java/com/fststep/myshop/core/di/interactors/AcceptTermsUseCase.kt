...
/**
 * Created by Shubham Singh on 30/08/21.
 */
class AcceptTermsUseCase @Inject constructor(private val apiService: ApiService) {

    operator fun invoke(token:String,userType:String,id:String) = flow<State<CommonResponse>> {
        apiService.acceptTerms(token,userType,id).run {
